...
def preprocess_sem(img):
    """High-pass spatial filtering to strip SEM surface charging & brightness gradients."""
...
def predict_center(search_path, ref_path, model_path="drift_sense_model.pth"):
    ...
    # SEM Noise Preprocessing
    search_proc = preprocess_sem(search_raw)
    ref_proc = preprocess_sem(ref_raw)
    ...
    res_full = cv2.matchTemplate(search_proc, ref_100, cv2.TM_CCOEFF_NORMED) # (901, 901)

    # 3. Stage B: Focused Landmark Crop (60x60 Center Crop around defect)
    crop_size = 60
    offset = (100 - crop_size) // 2  # 20 px
    ref_crop = ref_100[offset:offset + crop_size, offset:offset + crop_size]

    res_crop = cv2.matchTemplate(search_proc, ref_crop, cv2.TM_CCOEFF_NORMED) # (941, 941)

    # Align Stage B response map to Stage A coordinates safely
    h_f, w_f = res_full.shape
    res_crop_aligned = res_crop[offset:offset + h_f, offset:offset + w_f]
    ...
    # 5. 2D Parabolic Peak Interpolation with Bounded Offsets
    dx, dy = 0.0, 0.0
    if 0 < lx < score_map.shape[1] - 1 and 0 < ly < score_map.shape[0] - 1:
        denom_x = (2.0 * score_map[ly, lx] - score_map[ly, lx + 1] - score_map[ly, lx - 1])
        if abs(denom_x) > 1e-5:
            dx = (score_map[ly, lx + 1] - score_map[ly, lx - 1]) / (2.0 * denom_x)
            dx = float(np.clip(dx, -0.5, 0.5))
            
        denom_y = (2.0 * score_map[ly, lx] - score_map[ly + 1, lx] - score_map[ly - 1, lx])
        if abs(denom_y) > 1e-5:
            dy = (score_map[ly + 1, lx] - score_map[ly - 1, lx]) / (2.0 * denom_y)
